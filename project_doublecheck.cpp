...
int Project_Doublecheck::BaseCase(){
	ifstream inFileLHS;
	ofstream outFileSummary;


	outFileSummary.open("project_doublecheck_output_basecase.txt");
...
int Project_Doublecheck::RunDifferentMCReplc(){
	ofstream outFileSummary;


	outFileSummary.open("project_doublecheck_output_mcreplc.txt");
	outFileSummary<<endl<<"===== "<<currentDateTime()<<" ====="<<endl
...
int Project_Doublecheck::CEA_BaseResults()
{
...
int Project_Doublecheck::CEA_Horizon(int argIdx) {
	

	//double listHorz[] = { 5,10,15,20,25,30,35,40,50 };
	//TIME_HORIZON = listHorz[argIdx];
	TIME_HORIZON = argIdx + 1;
...
int Project_Doublecheck::Compare(string strArm1, string strArm2, int argGenotype)
{
	...
	for(int k=0; k<nScenarios; k++){
		...
		_outFileSummary<<TIME_HORIZON<<"\t"<<r1[0]<<"\t"<<r2[0]<<"\t"<<r2[0]-r1[0]<<"\t"
		<<r1[1]<<"\t"<<r2[1]<<"\t"<<r2[1]-r1[1]<<"\t"
			<<(r2[1]-r1[1])/(r2[0]-r1[0])<<"\t";
...
vector<double> Project_Doublecheck::EvaluateOneArm( string strArm, const baseCohortType & testCohort )
{
...
vector<double> Project_Doublecheck::EvaluateOneArm_CombineLDV8Or12WkOnline(string txArm, const baseCohortType & testCohort )
{
...
int Project_Doublecheck::CEA_OneWay(int argCmpIdx)
{

	

	ofstream outf_low, outf_high;
	outf_low.open("output_project_doublecheck/SA_LOW_"+_listCmp[argCmpIdx]+".txt");
	outf_high.open("output_project_doublecheck/SA_HIGH_"+_listCmp[argCmpIdx]+".txt");
	outf_low<< fixed << showpoint;
	outf_high<< fixed << showpoint;

	vector<string> sa_varName;
	vector<SParamTriplet> sa_varVal;
	ReadOneWaySAParamValueRange("project_doublecheck_input_OneWay.txt",sa_varName,sa_varVal);
...
int Project_Doublecheck::ReadPSASampledValues( ifstream & inf, modelParamType & argModelParam )
{
...
int Project_Doublecheck::CEA_PSA(int argCmpIdx, int argBatch, int argBatchSize)
{
	ofstream outf_psa;
...
int Project_Doublecheck::ChangeValue(string varName, double argVal, modelParamType & argModelParam){
...
Project_Doublecheck::Project_Doublecheck()
{
