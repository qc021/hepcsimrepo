#include "project_3wk.h"
using namespace std;




int Project_3Wk::BaseCase(){
	ifstream inFileLHS;
	ofstream outFileSummary;


	outFileSummary.open("project_3wk_output_basecase.txt");
...
int Project_3Wk::RunDifferentMCReplc(){
	ofstream outFileSummary;


	outFileSummary.open("project_3wk_output_mcreplc.txt");
	outFileSummary<<endl<<"===== "<<currentDateTime()<<" ====="<<endl
...
int Project_3Wk::CEA_BaseResults()
{
	//-Target population: G1b, non-cirrhotic, TN
	//	-SVR 100%
	//	-Evaluate the CEA of each of these regimens and compare them with the existing therapies we have for G1b in the model:
	...
	Compare("TN_TOL_G1_BOC_TEL","TN_TOL_G1b_SOF_LDV_ASV3",12);
	Compare("TN_TOL_G1_BOC_TEL","TN_TOL_G1b_SOF_DCV_SMV3",12);
	Compare("TN_TOL_G1_BOC_TEL","TN_TOL_G1b_SOF_DCV_ASV3",12);
	...
	Compare("NoTx","TN_TOL_G1b_SOF_LDV_ASV3",12);
	Compare("NoTx","TN_TOL_G1b_SOF_DCV_SMV3",12);
	Compare("NoTx","TN_TOL_G1b_SOF_DCV_ASV3",12);
...
vector<double> Project_3Wk::EvaluateOneArm_Averaged(string txArm, const baseCohortType & testCohort )
{
	vector<double> r;
...
int Project_3Wk::Compare(string strArm1, string strArm2, int argGenotype)
{


	_outFileSummary.open("project_3wk_output_compare.txt",ios::app);
...
vector<double> Project_3Wk::EvaluateOneArm( string strArm, const baseCohortType & testCohort )
{
...
vector<double> Project_3Wk::EvaluateOneArm_CombineLDV8Or12WkOnline(string txArm, const baseCohortType & testCohort )
{
...
int Project_3Wk::CEA_OneWay(int argCmpIdx)
{

	

	ofstream outf_low, outf_high;
	outf_low.open("output_project_3wk/SA_LOW_"+_listCmp[argCmpIdx]+".txt");
	outf_high.open("output_project_3wk/SA_HIGH_"+_listCmp[argCmpIdx]+".txt");
	outf_low<< fixed << showpoint;
	outf_high<< fixed << showpoint;

	vector<string> sa_varName;
	vector<SParamTriplet> sa_varVal;
	ReadOneWaySAParamValueRange("project_3wk_input_OneWay.txt",sa_varName,sa_varVal);
...
int Project_3Wk::ReadPSASampledValues( ifstream & inf, modelParamType & argModelParam )
{
...
int Project_3Wk::CEA_PSA(int argCmpIdx, int argBatch, int argBatchSize)
{
	ofstream outf_psa;
	outf_psa.open("output_project_3wk/PSA_"+_listCmp[argCmpIdx]+"_"+basicToStr(argBatch)+".txt");
	outf_psa<< fixed << showpoint;

	ifstream inf;
	inf.open("project_3wk_input_PSA.txt");
	string line;
...
int Project_3Wk::ChangeValue(string varName, double argVal, modelParamType & argModelParam){
	if ("q_F0"== varName){argModelParam._qolData.q_F0 = argVal;
		...
		ExitWithMsg("[Error] Project_3Wk::ChangeValue(string varName, double argVal): Unknown parameter "+varName);
	}
	return 0;
}

Project_3Wk::Project_3Wk()
{
	_drugCostReduction=0;
	// read all arms for comparisons
	ifstream inf;
	inf.open("project_3wk_input_comparators.txt");
	string word;
	while(inf>>word){
